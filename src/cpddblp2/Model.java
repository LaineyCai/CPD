...
//	double[][] ind_fea;	//U * number of features; feature need normalize to keep in the same scale
	 ...
	 */
	protected boolean init(User[] users, int vocabSize, int tsNo,TIntObjectHashMap<int[]> myuserNeighbour, int[] myltRefIx,int[] myluRefIx, HashSet<String> mynoretweet) {
		U = users.length;
		V = vocabSize;
		M = tsNo;
...
/*					List<int[]> instances = new ArrayList<int[]>();
	...
	//					f=IndFea(v);
					...
					tweight = tweight/unitlength + popu;
					for(int a=0; a<A; a++)
					{
