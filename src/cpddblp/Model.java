...
public class Model {
	...
	public int N2f = 2*Nf;
	public int NLu = 3063186;	//number of user-user links
	public int NLt = 16613081;	//number of document-document links
...
//				{
					...
					{
						tRefIx+=users[u].getRetweets().get(n).length;
					}
//				}
			}
		}
...
//					f=IndFea(userInd);
	...
	public void printnoretweet(User[] users, String outputDir)
	{
		ArrayList<String> noretweet = new ArrayList<String>();
		for (int u = 0; u < U; u++) {
			for (int n = 0; n < users[u].getDocWords().length; n++) {
		...
		FileUtil.writeLines(outputDir, noretweet);
	}
...
//		SNTC[topic]++;
	...
	//					f=IndFea(v);
						...
						tDiscFuncVal = tDiscFun(NUC[u],NUC[v],users[u].getDocWords().length, users[v].getDocWords().length,SNTC[z],z,MatrixUtil.getColumn(NCT, z),popu);
	...
	{		
        ...
        Parameter parameter = new Parameter(solver, C,maxIter, eps);
	...
	private void drawNu()
	{
		int M = 2*(NLt/5), N = N2f+2;
		int halfM = M/2;
		Feature[][] featureMatrix = new Feature[M][N];
        for(int i=0; i<M/2; i++)
        {
        	featureMatrix[i][0] = new FeatureNode(1,cijp[i*5]);
        	featureMatrix[i][1] = new FeatureNode(2,popp[i*5]);

        	for(int j=0; j<N2f; j++)
        	{
        		featureMatrix[i][j+2]=new FeatureNode(j+3,tpf[i*5][j]);
        	}
        	featureMatrix[i+halfM][0] = new FeatureNode(1,cijn[i*5]);
        	featureMatrix[i+halfM][1] = new FeatureNode(2,popn[i*5]);

        	for(int j=0; j<N2f; j++)
        	{
        		featureMatrix[i+halfM][j+2]=new FeatureNode(j+3,tnf[i*5][j]);
        	}
        }
        ...
        {
        	targetValue[i] = 1;	//positive instances
        	targetValue[halfM+i]=-1;	//negative instances
        }
		...
		for(int u=0; u<U; u++)
		{
			...
			if(retweetinfo!=null)
			{
				...
				for(int utIx: retweetinfo.keys())
				{
					...
					{
						int v = p[i][0];	//the user where the tweet is from
						for (int j=0; j<Nf; j++)	//concat u and v's feature
						{
							tp[count][j] = indfea[u][j];
							tp[count][Nf+j] = indfea[v][j];
						}
						r.add(v);
						count++;
						indi++;
